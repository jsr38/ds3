...
class Hdf5ExchangeDatatype {
  ...
  byte[] toByteArray(EXCHANGE[] exchanges) {
    ...
    byte[] codeByteArray = new byte[EXCHANGE_DATATYPE_SIZE_CODE];
    byte[] nameByteArray = new byte[EXCHANGE_DATATYPE_SIZE_NAME];
    byte[] countryByteArray = new byte[EXCHANGE_DATATYPE_SIZE_COUNTRY];
    byte[] currencyByteArray = new byte[EXCHANGE_DATATYPE_SIZE_CURRENCY];
    byte[] suffixByteArray = new byte[EXCHANGE_DATATYPE_SIZE_SUFFIX];
    byte[] timezoneByteArray = new byte[EXCHANGE_DATATYPE_SIZE_TIMEZONE];
    byte[] isIntradayByteArray = new byte[EXCHANGE_DATATYPE_SIZE_ISINTRADAY];
    byte[] intradayStartDateByteArray = new byte[EXCHANGE_DATATYPE_SIZE_INTRADAYSTARTDATE];
    byte[] hasIntradayProductByteArray = new byte[EXCHANGE_DATATYPE_SIZE_HASINTRADAYPRODUCT];
