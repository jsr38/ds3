...
public class EodDataSinkFirstElementLessThanOneDayAfterFirstAvailable extends EodDataSinkMock {
  ...
  EodDataSinkFirstElementLessThanOneDayAfterFirstAvailable(UnitTestData testData) throws EodDataSinkException {
    super(testData);
  ...
  public void updateExchangeSymbolQuotes(String exchange,
    ...
    final Range<Calendar> existingRange = new Range<Calendar>(testQuotes[LOWER_INDEX_OFFSET].getDateTime(),
                                                              testQuotes[testQuotes.length + UPPER_INDEX_OFFSET].getDateTime());
    
    final Range<Calendar> newRange = new Range<Calendar>(quotes[0].getDateTime(),
                                                         quotes[quotes.length - 1].getDateTime());
    
    assertFalse("Quote vector overlaps existing quotes", existingRange.overlapsRange(newRange));
  ...
  public Range<Calendar> readExchangeSymbolDateRange(String exchange, String symbol) {
    ...
    final Calendar upper = quotes[quotes.length + UPPER_INDEX_OFFSET].getDateTime();
